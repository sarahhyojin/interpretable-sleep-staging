...
class LRP:
    ...
    # changed here
    def generate_LRP(self, input, index=None, method="transformer_attribution", is_ablation=False, start_layer=0, seq_idx=0):
        output = self.model(input)[seq_idx] # output has 15 sequences and epoch
        kwargs = {"alpha": 1}
        if index == None:
        ...
        return self.model.relprop(torch.tensor(one_hot_vector).to(input.device), method=method, is_ablation=is_ablation,
                                  start_layer=start_layer, seq_idx=seq_idx, **kwargs)
